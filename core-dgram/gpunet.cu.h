...
__device__ int single_thread_gbind_in(const sockaddr_in* addr, socklen_t addrlen);
__device__ int single_thread_gbind_bounce_in(const sockaddr_in* addr);

__device__ int gconnect(const char* name, int namelen);
__device__ int gconnect_in(int socket, const struct sockaddr_in* addr, const socklen_t addrle);
...
__device__ int gsendto(int sock, uchar* to_send, int size, struct sockaddr *destaddr, int addrlen);
__device__ int gsend_test(int sock, uchar* to_send, int size);

__device__ int grecv(int sock, uchar* to_recv, int size);
__device__ int grecvfrom(int sock, void* buf, int size, struct sockaddr *addr, socklen_t *addrlen);
